...
public class thuocDao extends abstractDao<thuoc> {
	...
	public List<thuoc> search(String txt){
		try {
			...
			String txtPlus = "%"+txt+"%";
			return query(sql, new tenThuocMapper() , txtPlus);
	...
	public List<thuoc> listAll(){
		...
		return query(sql, new thuocMapper());
	}
	public List<thuoc> listOfName(String name){
		String sql = "select * from thuoc where tenThuoc = ?";
		String namePlus = name.trim();
		return query(sql, new thuocMapper(), namePlus);
	}
	public List<thuoc> listOfLoai(int idlt){
		String sql ="select * from thuoc where idLoaiThuoc = ?";
		return query(sql, new thuocMapper(), idlt);
	}
